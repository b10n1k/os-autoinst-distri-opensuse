...
sub run ($self) {
    ...
    if ($mpi eq 'mvapich2') {
        my $return = script_run("set -o pipefail; mpirun -np 4 --host slave-node00,slave-node01 /usr/lib/hpc/gnu7/$mpi/imb/$imb_version/bin/IMB-MPI1 PingPong |& tee /tmp/mpi_bin.log", timeout => 120);
        if ($return == 136) {
            if (script_run('grep \'Caught error: Floating point exception (signal 8)\' /tmp/mpi_bin.log') == 0) {
                record_soft_failure('bsc#1175679 Floating point exception should be fixed on mvapich2/2.3.4');
            }
        ...
        assert_script_run("mpirun -np 4 --host slave-node00,slave-node01 /usr/lib/hpc/gnu7/$mpi/imb/$imb_version/bin/IMB-MPI1 PingPong");
    }
